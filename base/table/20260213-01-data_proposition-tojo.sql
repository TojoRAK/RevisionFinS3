...
CREATE OR REPLACE VIEW v_objet_proposition AS
SELECT p.* , o1.id idObj1 , o1.title wanted_title , o2.id idObj2 , o2.title offered_title , u1.name owner_name , u2.name requester_name
FROM proposition p 
JOIN objet o1 ON p.wanted_id = o1.id JOIN objet o2 ON p.offered_id = o2.id
JOIN users u1 ON p.owner_id = u1.id JOIN users u2 ON p.requester_id = u2.id; 

-- Images (une principale par objet)
-- INSERT INTO objet_image (objet_id, path, is_main) VALUES_

INSERT INTO proposition (requester_id , owner_id,wanted_id,offered_id,status) VALUES (6,7,1,6,'PENDING');